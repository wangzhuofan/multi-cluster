...
kl <- 8
#kclass <- vector()
c2left <- c2track[which(kclass==kl)]
...
pc3ppp = melt(t(c3), varnames = c("Factor", "thirdorder"), value.name = "Indicator")
```
...
```{r}
ishgene <- read.table("C://ISBD/bicluster code/0220/gene.txt",sep="\t",quote = "")
...
name_gene <- read.csv("C://ISBD/bicluster code/0228/namegene.csv")
...
```{r}
foo <- function() {
  Rcpp::sourceCpp("rand.cpp", rebuild = F) #assuming Cfoo function is in defined in foo.cpp file
  randarma1(2)
}
library(doRNG)
library(doSNOW)
...
registerDoSNOW(cl)
set.seed(123)
res <- foreach(i=1:5) %dorng% { foo() }
set.seed(123)
res2 <- foreach(i=1:5) %dorng% { foo() }
stopCluster(cl)
```
...
```{r}
for (i in 1:383) {
  heatmap(brain_array[,,i],main = paste("gene", brain_gene[i]))
}
```
